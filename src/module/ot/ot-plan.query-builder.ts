...
export class OtPlanQueryBuilder {
  ...
  applyAuthorization(
    ...
    if (user.role === UserRole.EMPLOYEE) {
      throw new ForbiddenException(
        'Bạn không có quyền xem Danh sách Kế hoạch OT',
      );
    }

    // Admin chỉ thấy plan của các phòng không phải IT (phòng IT do Lead IT duyệt)
    if (user.role === UserRole.ADMIN) {
      qb.andWhere('creator.departmentName != :itDept', {
        itDept: Department.IT,
      });
    }

    if (
      user.role === UserRole.DEPARTMENT_LEAD ||
      user.role === UserRole.PROJECT_COORDINATOR ||
  ...
  applyFilters(
    ...
    if (query.search) {
      ...
      qb.andWhere(
        '(CAST(creator.id AS CHAR) LIKE :search OR creator.name LIKE :search)',
        {
    ...
    if (query.month && query.year) {
      qb.andWhere(
        'MONTH(op.startTime) = :month AND YEAR(op.startTime) = :year',
        {
          month: query.month,
          year: query.year,
        },
