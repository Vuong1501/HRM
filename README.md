...
với quyền là hr, sẽ gửi invite từ hệ thống sang bên zoho mail, nhân viên mới sẽ accept, sau đó được login vào hệ thống qua zoho, còn
nếu không được invite thì sẽ không login qua zoho được
-HR invite nhân viên => tạo thông tin user tạm vào bảng user

mai sửa lại các type để nó không gạch chân báo lỗi
eslint nữa
đọc swagger, áp dụng swagger, trả ra các lỗi chuẩn của filter exception, entities ra dùng chung, dùng enum trong entities
các constain ra, đặt logger nữa

-đã config lại eslint
đang code lại các type cho chuẩn sau khi bỏ cmt eslint cảnh báo (chưa biết cách làm chỗ zohoStrategy) chỗ super, nên cmt hay viết là
export class ZohoStrategy extends PassportStrategy(
Strategy as unknown as new (...args: any[]) => any,
ZOHO_STRATEGY,
)
